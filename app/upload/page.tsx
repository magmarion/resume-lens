    ...
    return (
        <>
            ...
            <main className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6 pb-16 pt-24 sm:pt-22.5">
                ...
                <div className="anim-1 badge-pill mb-6 border-emerald-600/22 bg-emerald-600/10 sm:mb-7">
                    <div className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(74,158,143,0.9)]" />
                    <span className="text-[11px] font-semibold uppercase tracking-wider text-emerald-400">Step 1 of 4</span>
                    <span className="h-2.5 w-px bg-emerald-400/30" />
                ...
                <div className="anim-3 mt-9 flex flex-wrap items-center justify-center gap-2 sm:mt-10">
                    {[
                        ...
                        <div key={item.step} className="flex items-center gap-2">
                            ...
                            <div className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1.5 sm:px-3 ${item.active ? "border-emerald-600/25 bg-emerald-600/12" : "border-white/6 bg-transparent"
                                }`}>
                                <div className={`flex h-4.5 w-4.5 items-center justify-center rounded-full text-[10px] font-bold ${item.active ? "bg-emerald-400/25 text-emerald-400" : "bg-white/5 text-mist-950"
                                    }`}>
                                    {item.step}
                                </div>
                                <span className={`text-xs font-medium tracking-tight ${item.active ? "text-emerald-400" : "text-mist-950"}`}>
                                    {item.label}
                                </span>
                            </div>
                        </div>
